 ...
 * = Encyrpt =
 * *** password, salt, 
 * encryptThis(string $str). setSalt(String $salt)
 *   -> MD5
 *   -> SHA256
 *   -> SHA512
 *   -> saastonorsu
 ...
 * 
 * 
 *  * //// Implementing powerlevels (if needed)
 *   -> (subclass of users) PowerlevelUsers
 *   - hasAccessTo(Powerlevel $level)
 *      --> (subSubClasses)
 *          = Admin =
 *          - powerlevel
 *          = Guest =
 ...
 *   -> forumsModerator
 *   etc...
 * ///// These when / if you want to use powerlevels in the future /////
 */
 ...
 */    
    ...
    private $timeBetweenAttempts = 86400, // This defines, how much pause there is, when user fails to login to site and the user is "locked"
    ...
    */
    ...
    function __construct (mysqli $DB, $site, $cookieName = "", $DBUser_LevelMin = 0, $validCookieTime = 31536000, $maxAttempts = 5, $timeBetweenAttempts = 86400, $unactivatedAccount = 1) {
    ...
    function login ($user, $password, $stats = 0)
    {
        ...
        if(!($haku = $this->DBConn->query("SELECT u.ID, u.email, u.powerlevel, u.loginAttempts, u.attemptTime, u.IP FROM users u WHERE u.email='".$user."' AND u.password=PASSWORD('".$password."')"))) {
        ...
        if(!$tulos[1] || $tulos[2] < $this->DBUser_LevelMin || ($tulos[3] >= $this->maxAttempts && ($tulos[4]+$this->timeBetweenAttempts) < $this->time))
        {
            if (!$tulos[1]) {
        ...
        else
        {
            ...
            else
            {
                ...
                if($stats) {
                    $this->DBConn->query("INSERT INTO ".$this->DBStatsTableName." SET ".$this->DBStatsRealIPField."=".$this->getRealIpAddr().", ".$this->DBStatsTimestampField."='".$this->time."', ".$this->DBStatsUsernameField."='".$this->getUserName()."', ".$this->DBStatsUserIDField."='".$this->getUserID()."', sessID='".$this->cookieID."'");
                }
                return TRUE;
            }
        }
    ...
    //----------
    function authenticate ($cookieID = "", $loginPage=0) {
        // The validCookieTime is time + the validTime. So we have to reduce the time from the validCookieTime first and after that we can reduce it from the time:
...
//echo "SELECT u.ID, u.email, u.powerlevel, u.IP FROM users u, sessions s WHERE u.powerlevel > ".$this->DBUser_LevelMin." AND s.sessID='".$cookieID."' AND s.time >= '".$validTime."' AND s.userID = u.ID";
        $tulos = "";
        $kysely = "";
    ...
    function insufficientPowerLevel($attemptedPowerlevel)
    {
    ...
    function dontAllowLogins(int $value)
    {
        if($value==280) {
            return;
        }
    ...
    {
        $haku = "DELETE FROM sessions WHERE sessions.sessid='".$cookieID."'";
        $this->DBConn->query($haku);
        $haku = "INSERT INTO sessions SET time='".$this->time."', sessID='".$cookieID."', userID='".$user."'";
        $this->DBConn->query($haku);
        if(!$user){
